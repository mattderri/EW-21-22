...
\]
Higher orders of the expansion correspond to other contributions, operators with higher dimensionality, where the momentum corresponds to interactions which contain derivatives and act on currents.
\[
\begin{aligned}
&\frac{k^2}{m_W^4}\to\frac{1}{m_W^4}J_{had}^\mu\Box J_\mu^{lep} \quad &&D=8\\
&\frac{k^4}{m_W^6}\to\frac{1}{m_W^6}J_{had}^\mu\Box^2 J_\mu^{lep} \quad &&D=10
\end{aligned}
\]
...
\[
\pazocal{L}=\Bar{\Psi}(i\slashed{D}-M)\Psi-\frac{1}{4}(F_{\mu\nu})^2+\frac{1}{2}(\partial_\mu\phi)^2+y\phi\Bar{\Psi}\Psi-\frac{m_\phi^2}{2}\phi^2
\marginnote[-2cm]{\includegraphics{Images/aaphiphi.pdf}\\
Diagram of the full process.}
\]
We want to take the limit in which $E\ll M$, where $\Psi$ will be point-like. It is possible to obtain an effective theory where $\Psi$ is not present but it is integrated out. What we get is a contact interaction between $\phi$ and $A$ but since $\phi$ is neutral this interaction will not be of the type of minimal coupling but it will have to respect gauge invariance. \marginnote[-1cm]{\includegraphics[]{Images/phiplike.pdf}\\
...
\]
Suppose we want to expand it at 1-loop \marginnote{\includegraphics[]{Images/chiloop2.pdf}\\
1-loop expansion.}and for simplicity we consider $\chi$ massless, so the only parameter will be the external momenta:
\[
...
When these effective theories collapse? There is not a unique answer, we have to make some assumption. Let's see an example.
...
\]    
Suppose to measure the muon lifetime, what we measure is $c_6$. We understand that the process is mediated by effective operators, hence why we want to measure $c_6$, the ratio between the coupling and the scale.\marginnote{\includegraphics[]{Images/scalenp.pdf}}
\end{example} 
The range highlighted in red in the picture on the side denotes where this new physics could appear, but we do not know exactly where. How could we determine it? We either make some assumption on the value of the coupling and extrapolate the value of $m_W$ from there or we look for new physics in scattering processes with the same elements of the Fermi interaction, $e^-\nu_\mu\to\mu^-\nu_e$. This was done by the Gargamella experiment in 1973, observing both charged and neutral current, so $W$ and $Z$. To find the masses, we had to wait until 1982 at CERN.
\end{document}
