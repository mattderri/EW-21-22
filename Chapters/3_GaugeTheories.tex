...
We can impose $\Phi=\Phi^c$, throwing away 4 degrees of freedom since it gives us 4 conditions. We can define $\mathcal{H}:=\Phi$\marginnote[-1.75cm]{The amount of new notation introduced in this few lines:\\
\includegraphics[]{Images/parkour.jpg}\\
Parkour!} which transforms as $\mathcal{H}\to L\mathcal{H}R^\dagger$ and the reality condition translates as $\mathcal{H}=i\sigma^2\mathcal{H}^*(-i\sigma^2)$. From this condition it follows that $\mathcal{H}=\varphi(x)\Sigma(x)$, with $\varphi(x)$ a real scalar field. $\mathcal{H}$ has 8-4=4 degrees of freedom, 3 in $\Sigma(x)$ and 1 in $\varphi(x)$.\\
%1:22:30
In general $\mathcal{H}$ is a $2\times2$ matrix which can always be written as\\
$\mathcal{H}=a_0\mathbb{1}+ia_i\sigma^i$, but the reality condition implies that $a_0$ and $a_i$ must be real.
\begin{align*}
